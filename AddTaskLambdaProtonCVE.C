...
{  
  ...
  if (doNUE) {
    ...
    if (period.EqualTo("LHC18q")) {
      fNUEFile = TFile::Open("alien:///alice/cern.ch/user/c/chunzhen/CalibFiles/LHC15o/efficiencyBothpol18qnew.root","READ");
      fListNUE = dynamic_cast <TList*> (fNUEFile->Get("fMcEffiHij"));
    }
    if(fListNUE) {
  ...
  if (v0calibOn) {
    ...
    if (period.EqualTo("LHC18q")) {
      fVZEROCalibFile = TFile::Open("alien:///alice/cern.ch/user/c/chunzhen/CalibFiles/LHC15o/calibSpq2V0C18qP3.root","READ");
